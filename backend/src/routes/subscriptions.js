...
router.post('/', wrapAsync(async (req, res) => {
    ...
    await SubscriptionsService.create(req.body.data, req.currentUser, true, link.host);
    const payload = true;
...
router.put('/:id', wrapAsync(async (req, res) => {
  await SubscriptionsService.update(req.body.data, req.body.id, req.currentUser);
  const payload = true;
  res.status(200).send(payload);
}));
...
router.get('/', wrapAsync(async (req, res) => {
  ...
  const payload = await SubscriptionsDBApi.findAll(
...
router.get('/:id', wrapAsync(async (req, res) => {
  const payload = await SubscriptionsDBApi.findBy(
    { id: req.params.id },
  );
