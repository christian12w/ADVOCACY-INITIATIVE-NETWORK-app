...
const CardSubscriptions = ({
  ...
  return (
    <div className={'p-4'}>
      ...
      >
        {!loading && subscriptions.map((item, index) => (
          <li
            key={item.id}
          ...
          >
              <Link href={`/subscriptions/subscriptions-view/?id=${item.id}`} className='text-lg font-bold leading-6 line-clamp-1'>
                  {item.plan}
              </Link>
              ...
              <div className='ml-auto'>
                <ListActionsPopover
                  onDelete={onDelete}
                  itemId={item.id}
                  pathEdit={`/subscriptions/subscriptions-edit/?id=${item.id}`}
                  pathView={`/subscriptions/subscriptions-view/?id=${item.id}`}
            ...
            <dl className='divide-y dark:divide-dark-700 px-6 py-4 text-sm leading-6 h-64 overflow-y-auto'>
                ...
                <div className='flex justify-between gap-x-4 py-3'>
                    <dt className='text-gray-500 dark:text-dark-600'>Plan</dt>
                    <dd className='flex items-start gap-x-2'>
                        <div className='font-medium line-clamp-4'>
                            { item.plan }
                        </div>
